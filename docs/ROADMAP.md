...
- **WS3 — Core V0 research:** Selector V1 research is now
  `COMPLETED / CLOSED` under a frozen, research-only contract: Lifecycle V1.3
  context → C5 working screen → S2 structural-primary ranking → Top2. Earlier
  A1/A2 confirmed-breakout and expanded-universe artifacts remain bounded
  historical evidence and are not promoted. The latest Lifecycle-conditioned
  expectancy study remains descriptive evidence with bounded join coverage;
  none of these artifacts creates a strategy filter, score, threshold, OOS
  claim, or policy promotion. Selector V1 does not create a production
  selector or activate entry timing, sizing, risk regime, benchmark,
  execution, or transaction-cost policy. See [Selector V1 contract](research/selector-v1-contract.md)
  and the [closure report](../reports/TASK-WS3-CORE-V0-SELECTOR-V1-OWNER-FREEZE-CANONICAL-CONTRACT-MATERIALIZATION-AND-RESEARCH-SERIES-CLOSURE-20260830/formal-closure-report.md).
