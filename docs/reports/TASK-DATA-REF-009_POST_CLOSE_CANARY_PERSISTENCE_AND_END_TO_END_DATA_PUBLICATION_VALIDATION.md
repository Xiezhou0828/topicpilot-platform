# TASK-DATA-REF-009嚚ost-Close Canary Persistence & End-to-End Data Publication Validation

## Scope and preserved authority

This task follows the already completed and preserved G0?3 checkpoints. It
...
## Exact-SHA release handoff

```text
IMPLEMENTATION_SHA = edfeb0e59c53ccf957d2b100a4f4ec619f67b519
NON_FORCE_BRANCH_PUSH = YES
PULL_REQUEST = https://github.com/Xiezhou0828/topicpilot-platform/pull/3
EXACT_SHA_CI_RUN = 31771593422
EXACT_SHA_CI = PASS
DEPLOY_RUN = 31771782436
DEPLOY = PASS (API only)
APPLICATION_RUNTIME_AUTHORITY_SHA = PENDING OPERATOR VERIFICATION
DOCUMENTATION_SHA = NONE AFTER DEPLOYMENT; local closure updates are not deployed
PRODUCTION_MUTATION = NO
CANARY = NOT_RUN
FINAL_STATUS = READY_FOR_RUNTIME_PROVENANCE_AND_ONE_SHOT_CANARY_REVIEW
BLOCKER = Awaiting same-runtime SHA/config evidence and authorized Canary result.
```

## Production runtime evidence and fail-closed stop

The deployed application runtime was verified in the same Render Shell:
...
G0 = PASS
TPE_PROVIDER = TWSE_OFFICIAL_DAILY / twse-official-daily.v2
TWO_PROVIDER = TPEX_OFFICIAL_DAILY / tpex-official-daily.v2
MARKET_BATCH = true
...
The read-only live dry-run showed `referenceDataVersion=tw-reference-v1`.
The already-activated Production registry authority is
`tw-reference-v1-rollover-daf19e9eb051255c`; therefore the post-close
date-effective reference precondition correctly failed closed with
`REFERENCE_CONTEXT_NOT_READY` before creating a collector run or reaching
the provider/persistence writer.

```text
CANARY_COMMAND_ATTEMPTED = topicpilot-live --mode post-close --once --run-date 2026-08-13
CANARY_ATTEMPTS = 1 (precondition blocked)
...
Repository authority confirms the live runtime config defaults to
`TOPICPILOT_LIVE_REFERENCE_DATA_VERSION` or `tw-reference-v1`, while the
active registry was intentionally transitioned to the rollover version in
TASK-DATA-REF-006I. This is a runtime reference-version binding blocker, not
a provider coverage, lifecycle, or Canary persistence failure. Do not retry
the Canary until the service configuration is explicitly rebound to the
active registry and the resulting runtime provenance is reverified.

```text
FINAL_STATUS = BLOCKED_REFERENCE_CONTEXT_NOT_READY
BLOCKER = Runtime referenceDataVersion tw-reference-v1 does not match the
          active Production registry tw-reference-v1-rollover-daf19e9eb051255c.
```
