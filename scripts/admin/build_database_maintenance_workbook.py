...
def _load_snapshot(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {
            ...
            "canonical_migration_head": "UNKNOWN",
...
def _readme_rows(snapshot: dict[str, Any]) -> list[list[Any]]:
    ...
    rows = [
        ...
        ["DATA_SOURCE", source],
        ["DATA_ENVIRONMENT", snapshot.get("environment", "UNAVAILABLE")],
        ["SNAPSHOT_TIMESTAMP", snapshot.get("timestamp", "")],
        [
            "SOURCE_MIGRATION_HEAD",
            snapshot.get("source_migration_head") or "UNKNOWN",
        ],
        [
            "CANONICAL_MIGRATION_HEAD",
            snapshot.get("canonical_migration_head") or "UNKNOWN",
        ],
        [
            "SOURCE_SCHEMA_STATUS",
            snapshot.get("source_schema_status", "UNKNOWN"),
        ],
        [
            "RELATION_WEIGHT_SOURCE_STATUS",
            "AVAILABLE" if snapshot.get("relation_weight_available") else "UNKNOWN / UNAVAILABLE",
        ],
        [
            "Snapshot counts",
