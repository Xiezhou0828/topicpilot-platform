...
- **WS3 — Core V0 research:** A1 quality-filter confirmatory evidence is
  ...
  The Selector V1 research series is now `CLOSED` with a frozen,
  research-only contract: `Lifecycle V1.3 topic context → C5 candidate screen
  → S2 within-topic ranking → Top2`. C5 remains a working baseline rather
  than universal screen authority; S2 uses `distance_to_ma60` as the primary
  structural rank and `volatility_20d` only for an exact structural tie.
  Production selector creation/activation, entry timing, position sizing,
  risk regime, benchmark, execution, and transaction-cost contracts remain
  outside this closure. See the [Selector V1 canonical contract](docs/research/selector-v1-contract.md)
  and [Owner freeze closure](reports/TASK-WS3-CORE-V0-SELECTOR-V1-OWNER-FREEZE-CANONICAL-CONTRACT-MATERIALIZATION-AND-RESEARCH-SERIES-CLOSURE-20260830/formal-closure-report.md).
