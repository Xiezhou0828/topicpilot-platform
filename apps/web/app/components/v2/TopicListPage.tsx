...
import { fetchTopics, getTopicPublication, lifecycleStageAvailable, lifecycleStatusLabel, scoreLabel, type TopicLifecycle, type TopicPublicationDisclosure, type TopicResource, type TopicSource, type TopicSummary } from "../../lib/topic-api";
import { ownerStageFromBackend, type OwnerLifecycleStage } from "../../lib/topic-lifecycle-contract";
import { getTopicOverviewLifecycle, getTopicOverviewMeta, PREVIEW_LABEL, type TopicGrade, type TopicLifecycleStage, type TopicOverviewMeta } from "../../lib/topic-preview";
...
type DisplayLifecycleStage = OwnerLifecycleStage;
...
function displayLifecycleStage(stage: TopicLifecycleStage | DisplayLifecycleStage): DisplayLifecycleStage | null {
...
function lifecycleForTopic(topic: OverviewTopic, preview: boolean): LifecycleTopic["lifecycle"] | null {
  if (!preview) {
    if (!topic.lifecycle || !lifecycleStageAvailable(topic.lifecycle.dataStatus)) return null;
    const stage = formalLifecycleStage(topic.lifecycle.currentStage);
    return stage ? { stage, day: topic.lifecycle.currentStageTradingDays, status: topic.lifecycle.dataStatus, transition: topic.lifecycle.transitionDecision ?? topic.lifecycle.transitionReason ?? null } : null;
  }
  if (topic.lifecycle && topic.lifecycle.dataStatus !== "PREVIEW") return null;
  const previewLifecycle = getTopicOverviewLifecycle(topic.slug);
  const stage = displayLifecycleStage(previewLifecycle.stage);
  return stage ? { stage, day: previewLifecycle.day, status: "PREVIEW", transition: null } : null;
}
...
function LifecycleChip({ item }: { item: LifecycleTopic }) {
  return <Link href={`/topics/${item.topic.slug}`} className="tp-topic-lifecycle-chip">
    <span><b>{item.topic.name}</b><small>{item.lifecycle.day === null ? "目前階段由 API 提供" : `Day ${item.lifecycle.day}`} · {lifecycleStatusLabel(item.lifecycle.status)}{item.lifecycle.transition ? ` · ${item.lifecycle.transition}` : ""}</small></span>
    <strong>{scoreLabel(item.topic.score)}</strong>
  </Link>;
}

function TopicLifecycle({ topics, preview, source }: { topics: OverviewTopic[]; preview: boolean; source: TopicSource }) {
 const allowPreview = preview;
 const [showHelp, setShowHelp] = useState(false);
 const [expandedStage, setExpandedStage] = useState<DisplayLifecycleStage | null>(null);
  const disclosureTopic = topics.find((topic) => topic.lifecycle?.dataStatus === "SHADOW_AVAILABLE") ?? topics[0];
  const lifecycleDisclosure = disclosureTopic
    ? getTopicPublication(source, disclosureTopic).lifecycle
    : { field: "lifecycle" as const, state: "DEFERRED" as const, note: "正式 Lifecycle 尚未發布。" };
  ...
  const availability = useMemo(() => {
    const counts = new Map<TopicLifecycle["dataStatus"], number>();
    topics.forEach((topic) => {
      const status = topic.lifecycle?.dataStatus;
  ...
  return <section className="tp-topic-lifecycle-section" aria-labelledby="topic-lifecycle-title">
    ...
    <div className="tp-topic-lifecycle-availability" aria-label="Lifecycle 資料可用性"><span>Backend Lifecycle availability</span>{availability.map(([status, count]) => <span className="tp-chip" data-lifecycle-status={status} key={status}>{lifecycleStatusLabel(status)} {count}</span>)}</div>
  </section>;
}
  ...
  return <AppShell currentPath="/topics"><PageContainer title="題材" hideHeader className="tp-topic-overview-page">
    ...
    {resource?.data && <>
      <section className="tp-topic-kanban-section" aria-labelledby="topic-kanban-title">
        <div className="tp-topic-section-heading tp-topic-map-heading"><div><h2 id="topic-kanban-title">今日題材地圖</h2></div><div className="tp-topic-map-tools"><div className="tp-topic-filter-group" role="group" aria-label="今日方向篩選">{(["全部", "轉強", "轉弱"] as DirectionFilter[]).map((item) => <button type="button" key={item} className={directionFilter === item ? "is-active" : ""} onClick={() => setDirectionFilter(item)}>{item}</button>)}</div>{previewMode ? <PreviewBadge /> : publication && <PublicationDisclosure disclosure={publication.snapshot} />}</div></div>
      ...
      <TopicLifecycle topics={overviewTopics} preview={previewMode} source={resource.source} />
