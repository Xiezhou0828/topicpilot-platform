...
const label = (value: string | null) => value?.trim() || "角色尚未發布";
...
function availabilityMessage(eod: StockEodRead | null | undefined): string | null {
  if (eod?.dataStatus === "SUSPENDED") return "目前暫停報價／暫停交易";
  if (eod?.dataStatus === "UNAVAILABLE") return "今日報價暫不可用";
  return null;
}
...
export function StockEncyclopediaView({ stock: displayStock, onClose, presentation = "overlay", isClosing = false, detailState, detailError, onRetry }: ViewProps & {
  ...
  const quoteAvailability = availabilityMessage(displayStock.eod);
  ...
  const drawer = <aside className={`tp-stock-encyclopedia-drawer ${presentationClass}`} role="dialog" aria-modal={presentation === "overlay" ? true : undefined} aria-labelledby="stock-encyclopedia-title" onClick={(event) => event.stopPropagation()}>
    ...
    <div className="tp-stock-encyclopedia-body" data-stock-read-state={commercialState}>
      ...
      <div className="tp-stock-encyclopedia-price"><strong>{quote.price === null ? "—" : quote.price.toLocaleString("zh-TW", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>{quoteAvailability ? <span className="tp-muted" data-quote-availability={eodStatus}>{quoteAvailability}</span> : quote.changePct === null ? <span className="tp-muted">漲跌資料待更新</span> : <span className={`tp-topic-change tp-topic-change--${tone}`}>{displayPercent(quote.changePct)}</span>}</div>
      <section className="tp-stock-encyclopedia-section tp-stock-eod-read" data-eod-status={eodStatus} aria-label="正式 EOD 資料"><div className="tp-stock-encyclopedia-section-heading"><h3>EOD 行情</h3><span>{statusLabel(eodStatus)}</span></div>{displayStock.eod && !displayStock.isPreview ? <><EvidenceGrid rows={[["收盤價", displayStock.eod.close], ["前收／最後正式收盤", displayStock.eod.previousClose], ["漲跌", displayStock.eod.change], ["漲跌幅", displayStock.eod.changePct], ["開盤", displayStock.eod.open], ["最高", displayStock.eod.high], ["最低", displayStock.eod.low], ["成交量", displayStock.eod.volume], ["成交金額", displayStock.eod.turnover], ["狀態日期", displayStock.eod.tradingDate], ["最後正式交易日", displayStock.eod.lastFormalTradingDate], ["原因", displayStock.eod.availabilityReason]]} /><div className="tp-stock-eod-lineage"><span>價格來源：{sourceLabel(displayStock.eod.priceSource)}</span><span>成交量來源：{sourceLabel(displayStock.eod.volumeSource)}</span><span>證據：{displayStock.eod.availabilityEvidenceId ?? "正式行情 lineage"}</span><span>除權息：{displayStock.eod.adjustmentState}</span><span>觀測：{displayStock.eod.observedAt ?? "尚未提供"}</span><span>讀取：{displayStock.eod.retrievedAt ?? "尚未提供"}</span></div></> : <p className="tp-stock-encyclopedia-muted">{displayStock.isPreview ? "Preview 不代表正式 EOD；正式 API 可用後才顯示。" : "正式 EOD 尚未提供；不以盤中值、歷史資料或 Preview 補值。"}</p>}</section>
