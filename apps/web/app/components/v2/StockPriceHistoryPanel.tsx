...
export function historyStatus(data: StockHistoryRead): HistoryStatus {
  if (data.items.length === 0 || data.coverageState === "EMPTY") return "EMPTY";
  return data.status === "AVAILABLE" ? "AVAILABLE" : "UNAVAILABLE";
}
...
function HistoryTable({ items }: { items: StockHistoryPoint[] }) {
  return <div className="tp-stock-history-table-wrap">
    <table className="tp-stock-history-table">
      ...
      <thead><tr><th scope="col">Date</th><th scope="col">Open</th><th scope="col">High</th><th scope="col">Low</th><th scope="col">Close</th><th scope="col">Volume</th><th scope="col">Adjustment</th></tr></thead>
      <tbody>{items.map((point) => <tr key={`${point.tradingDate}-${point.observedAt}`}>
        ...
        <td>{formatNumber(point.volume)}</td><td>{point.adjustmentState ?? "UNKNOWN"}</td>
      </tr>)}</tbody>
    </table>
  </div>;
}
...
export function StockPriceHistoryPanel({
  ...
  return <StockHistoryContent data={data} status={status} error={state.error} />;
}

export function StockHistoryContent({ data, status, error }: { data: StockHistoryRead | null; status: HistoryStatus; error: string | null }) {
  const firstPoint = data?.items[0] ?? null;
  const adjustmentState = data?.items.length && data.items.every((point) => point.adjustmentState === firstPoint?.adjustmentState) ? firstPoint?.adjustmentState ?? "UNKNOWN" : "UNKNOWN";
  ...
  return <section className="tp-stock-encyclopedia-section tp-stock-price-history" data-history-status={status} aria-labelledby="stock-price-history-title">
    ...
    {status === "ERROR" && <p className="tp-stock-encyclopedia-muted">Historical price history could not be loaded. {error ? "請重新開啟個股重試。" : "The formal API returned an error."}</p>}
