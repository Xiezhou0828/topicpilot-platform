    ...
    <PageContainer eyebrow={UI.eyebrow} title={UI.title} description={UI.description}>
      ...
      <section className="tp-stock-workspace tp-stock-workspace--push" data-detail-state={detailPanelState}>
        <div className="tp-stock-main">
          ...
          {resource === null ? <Card><EmptyState title={UI.loading} description="PostgreSQL-backed Stock read model" /></Card> : resource.source === "unavailable" ? <Card><EmptyState title={UI.unavailable} description={resource.error ?? UI.unavailableDescription} /></Card> : displayRows.length === 0 ? <Card><EmptyState title={UI.noRows} description={UI.unavailableDescription} /></Card> : <div className="tp-stock-grid">{displayRows.map((stock) => { const quote = selectStockQuote(stock); const favorite = isFavorite(stock.code, stock.market); const availabilityCopy = quote.dataStatus === "SUSPENDED" ? "目前暫停報價／暫停交易" : quote.dataStatus === "UNAVAILABLE" ? "今日報價暫不可用" : null; return <div className="tp-stock-tile-wrap" key={stock.code}><button type="button" className={`tp-stock-tile ${selected?.code === stock.code ? "is-selected" : ""} ${isLive(stock) ? "" : "is-eod"}`} onClick={() => openDetailPanel(stock)} aria-label={`${stock.name} ${stock.code}`}><span className="tp-stock-tile-top"><strong>{stock.name}</strong><em>{isLive(stock) ? UI.live : UI.eod}</em></span><small>{stock.code} · {stock.market} · {stock.topics[0]?.name ?? UI.noTopic}</small><span className="tp-stock-tile-quote"><b>{formatPrice(quote.price)}</b><i className={quote.changePct === null ? "tp-muted" : quote.changePct < 0 ? "tp-stock-down" : "tp-stock-up"}>{formatChange(quote.changePct)}</i></span>{!isLive(stock) && <span className="tp-stock-eod-note">{stock.isPreview ? "Preview" : availabilityCopy ?? `${UI.postClose} · ${quote.dataStatus}`}</span>}</button><FavoriteStar active={favorite} onClick={(event) => { event.stopPropagation(); toggleFavorite(stock.code, { market: stock.market, displayLabel: stock.name }); }} /></div>; })}</div>}
        </div>
