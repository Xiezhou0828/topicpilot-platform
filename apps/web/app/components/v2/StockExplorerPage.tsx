...
export default function StockExplorerPage() {
  ...
  const [lastSorted, setLastSorted] = useState("尚未提供");
      ...
      <section className="tp-stock-workspace tp-stock-workspace--push" data-detail-state={detailPanelState}>
        <div className="tp-stock-main">
          <Card className="tp-stock-toolbar">
            ...
            <label><span>{UI.sort}</span><select value={sort} onChange={(event) => { setSort(event.target.value as SortKey); setLastSorted(new Date().toLocaleTimeString("zh-TW", { hour: "2-digit", minute: "2-digit" })); }}><option value="change">{UI.change}</option><option value="price">{UI.price}</option><option value="volume">{UI.volume}</option></select></label>
            ...
            <button type="button" className="tp-stock-resort" onClick={() => { setLastSorted(new Date().toLocaleTimeString("zh-TW", { hour: "2-digit", minute: "2-digit" })); void loadFormal(formalQuery); }}>{UI.resort}</button>
          </Card>
