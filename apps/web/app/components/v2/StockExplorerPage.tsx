...
export default function StockExplorerPage() {
  ...
  const loadFormal = useCallback(async (query: StockListQuery) => {
    ...
    if (next.source === "api" && next.data) {
      const incomingCodes = next.data.map((item) => `${item.market}:${item.code}`);
  ...
  const baseRows = useMemo(() => {
    ...
    return resource?.source === "synthetic-snapshot" && bundle.source === "snapshot" ? bundle.stockUniverse.map(fromPreview) : [];
  ...
  const filteredRows = useMemo(() => baseRows.filter((row) => {
    ...
    return (market === "all" || row.market === market)
      ...
      && chipMatch
      && strategyMatch;
  }), [baseRows, chip, isFavorite, market, mode, resource?.source, strategy, topic]);

  const displayRows = useMemo(() => {
    if (resource?.source !== "api") return [...filteredRows].sort((a, b) => compareRows(a, b, sort));
    const byCode = new Map(filteredRows.map((row) => [`${row.market}:${row.code}`, row]));
    const ordered = formalOrder.flatMap((code) => {
      const row = byCode.get(code);
      return row ? [row] : [];
    });
    const known = new Set(ordered.map((row) => `${row.market}:${row.code}`));
    ordered.push(...filteredRows.filter((row) => !known.has(`${row.market}:${row.code}`)));
    return ordered;
  ...
  return <AppShell currentPath="/stocks">
    <PageContainer eyebrow={UI.eyebrow} title={UI.title} description={UI.description}>
      <div className="tp-stock-meta">
        <Freshness state="資料待更新" asOf={resource?.source === "api" ? UI.formalReadModel : "Preview snapshot"} />
      ...
      <section className="tp-stock-workspace tp-stock-workspace--push" data-detail-state={detailPanelState}>
        <div className="tp-stock-main">
          ...
          {advancedOpen && <Card className="tp-stock-advanced">
            ...
            <label><span>{UI.technical}</span><select value={technical} disabled onChange={(event) => setTechnical(event.target.value as FilterValue)} aria-label={`${UI.technical} · ${UI.unavailableFilter}`}><option value="all">{UI.unavailableFilter}</option></select></label>
          ...
          {resource === null ? <Card><EmptyState title={UI.loading} description="PostgreSQL-backed Stock read model" /></Card> : resource.source === "unavailable" ? <Card><EmptyState title={UI.unavailable} description={resource.error ?? UI.unavailableDescription} /></Card> : displayRows.length === 0 ? <Card><EmptyState title={UI.noRows} description={UI.unavailableDescription} /></Card> : <div className="tp-stock-grid">{displayRows.map((stock) => { const quote = selectStockQuote(stock); const favorite = isFavorite(stock.code, stock.market); return <div className="tp-stock-tile-wrap" key={`${stock.market}:${stock.code}`}><button type="button" className={`tp-stock-tile ${selected?.code === stock.code && selected?.market === stock.market ? "is-selected" : ""} ${isLive(stock) ? "" : "is-eod"}`} onClick={() => openDetailPanel(stock)} aria-label={`${stock.name} ${stock.code}`}><span className="tp-stock-tile-top"><strong>{stock.name}</strong><em>{isLive(stock) ? UI.live : UI.eod}</em></span><small>{stock.code} · {stock.market} · {stock.mainTopic?.name ?? "主要題材未發布"} · {stock.topics.length} 個關係</small><span className="tp-stock-tile-quote"><b>{formatPrice(quote.price)}</b><i className={quote.changePct === null ? "tp-muted" : quote.changePct < 0 ? "tp-stock-down" : "tp-stock-up"}>{formatChange(quote.changePct)}</i></span>{!isLive(stock) && <span className="tp-stock-eod-note">{stock.isPreview ? "Preview" : quote.dataStatus === "UNAVAILABLE" ? "EOD 資料尚未提供" : `${stock.dataDate ?? "交易日待提供"} · ${quote.dataStatus}`}</span>}{!stock.isPreview && <small>來源：{stock.eod?.priceSource?.sourceCode ?? "尚未提供"} · {stock.dataFreshness ?? "UNKNOWN"}</small>}</button><FavoriteStar active={favorite} onClick={(event) => { event.stopPropagation(); toggleFavorite(stock.code, { market: stock.market, displayLabel: stock.name }); }} /></div>; })}</div>}
        </div>
