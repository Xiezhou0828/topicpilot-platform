...
function LifecyclePreview({ preview }: { preview: TopicPreview }) {
  ...
  return <Card className="tp-topic-detail-card tp-topic-detail-lifecycle-card"><TopicSectionHeading title="題材生命週期" description="以單一路徑閱讀題材目前位置；正式歷史與交易日欄位尚未接入。" /><div className="tp-topic-detail-lifecycle-current"><span>目前階段</span><strong>{current}</strong><b>{preview.lifecycle.entered}</b><span>{preview.lifecycle.duration}</span></div><ol className="tp-topic-detail-lifecycle-track">{LIFECYCLE_STAGES.map((stage) => { const segment = preview.lifecycle.segments.find((item) => displayLifecycleStage(item.stage) === stage); const active = current === stage; return <li className={active ? "is-active" : ""} key={stage}><span className="tp-topic-detail-lifecycle-dot" aria-hidden="true" /><strong>{stage}</strong><small>{segment?.entered ?? "—"}</small><em>{segment?.duration ?? "—"}</em></li>; })}</ol></Card>;
}

function TopicStatusPreview({ preview }: { preview: TopicPreview }) {
  const items = [["族群表現", preview.metrics.participation], ["領漲核心", preview.metrics.leaderDrive], ["動能擴散", preview.metrics.leaderConsistency]];
  return <Card className="tp-topic-detail-card tp-topic-status-card"><TopicSectionHeading title="題材狀態" description="三個研究視角保留為獨立欄位；正式 Read Model 尚未提供時不在瀏覽器端推導。" /><div className="tp-topic-status-grid">{items.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong><small>Preview · 等待正式 Read Model</small></div>)}</div></Card>;
}
