...
function TopicStatusSection({ topic, preview, formal }: { topic: TopicData; preview: TopicPreview; formal: boolean }) {
  const items = formal
...
export default function TopicDetailPage({ slug }: { slug: string }) {
  ...
  return <AppShell currentPath="/topics"><PageContainer className="tp-topic-page" title={topic?.name ?? slug} hideHeader><div className="tp-topic-detail-page">
    ...
    {topic && preview && <>
      ...
      <div className="tp-topic-content"><LifecyclePreview preview={preview} /><TopicStatusSection topic={topic} preview={preview} formal={resource.source === "api" && topic.status.length === 3} />
