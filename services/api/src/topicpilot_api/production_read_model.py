...
_LIFECYCLE_STATUS_SQL = ", ".join(f"'{status}'" for status in sorted(NO_TRADE_STATUSES))
        ...
        WHERE co.family_code = 'PRICE'
          ...
          AND NOT EXISTS (
              ...
              WHERE lifecycle.instrument_id = co.instrument_id
                AND lifecycle.status_code IN ({_LIFECYCLE_STATUS_SQL})
...
def _stock_eod(row: Any) -> dict[str, Any] | None:
    ...
    no_trade = status_code in COVERED_NO_TRADE_STATUS_CODES
...
def _lifecycle_unavailable() -> dict[str, Any]:
    return {
        ...
        "daysSinceMeaningfulExpansion": None,
