...
def _candidate(
    ...
    if value.previous_stage == MATURE and (
        ...
        and policy.mature_min_positive_breadth <= (metrics.lead_core_positive_breadth or 0.0)
...
def _structural_breakdown_repaired(
    ...
    return bool(
        ...
        and (metrics.lead_core_weak_ratio or 0.0)
        <= policy.structural_breakdown_repair_max_weak_ratio
        and bool(progression.get("trajectoryRecovered") or progression.get("meaningfulExpansion"))
    )
...
def evaluate_lifecycle(
    ...
    breakdown_now = _structural_breakdown_trigger(metrics, value, progression, active_policy)
    breakdown_repaired = _structural_breakdown_repaired(metrics, value, progression, active_policy)
    decline_repaired = _declining_repaired(metrics, value, progression, active_policy)
    ...
    if breakdown_created:
        ...
        breakdown_evidence = prior_memory.get("structuralBreakdownEvidence")
        breakdown_disposition = "PENDING_NEXT_VALID_SESSION" if prior_breakdown_pending else "NONE"
