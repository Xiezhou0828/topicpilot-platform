...
class UnitOfWork:
    def __init__(self, session: Session): self.session = session; self.repositories = RepositoryRegistry(session)
    def commit(self):
        try: self.session.commit()
        except Exception as exc: self.session.rollback(); raise PersistenceError("transaction commit failed") from exc
...
def unit_of_work(factory: sessionmaker[Session]) -> Iterator[UnitOfWork]:
    with UnitOfWork(factory()) as uow: yield uow
