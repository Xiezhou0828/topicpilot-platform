...
def test_instrument_topic_relation_schema_and_relationships():
    ...
    assert {c.name for c in inspect(InstrumentTopicRelation).columns} >= {
        "instrument_id",
        "topic_id",
        "relation_type",
        "relation_version",
    ...
    relation = InstrumentTopicRelation(
        ...
        relationship_metadata={"basis": "governed_fixture"},
    )
    assert relation.instrument is instrument and relation.topic is topic
    assert instrument.topic_relationships == [relation] and topic.instrument_relationships == [
        relation
    ]
